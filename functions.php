...
function contact_form_insert_data($name, $email, $phone, $message){
    global $wpdb;
    $contact_form_table = $wpdb->prefix . "contact_form";
...
function handle_contact_form_submission() {
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_contact_form'])) {
        ...
        if (isset($_POST['name'], $_POST['email'], $_POST['message']) && !empty($_POST['name']) && !empty($_POST['email']) && !empty($_POST['message'])) {
            ...
            contact_form_insert_data($_POST['name'], $_POST['email'], $_POST['phone'], $_POST['message']);
        ...
        // Validate Phone (Optional - Customize as needed)
        if (!empty($phone) && !preg_match('/^\+?[0-9]{10,15}$/', $phone)) {
            echo 'Please enter a valid phone number (including country code, if applicable).<br>';
            return;
        }
...
// Define the contact_form_get_data() function globally
function contact_form_get_data() {
    global $wpdb;
    $contact_form_table = $wpdb->prefix . "contact_form";
    return $wpdb->get_results("SELECT * FROM $contact_form_table");
}
                ...
                <?php
                    foreach( $data as $row ){
                        echo "<tr>
                            ...
                            <td>{$row->message}</td>
                            <td>  
                                <a href='?page=contact-form&action=delete&id={$row->id}'
...
?>
<!--Delete Programs from the table start -->
<?php 
    function contact_form_data_delete( $id ){
        global $wpdb;
        $contact_form_table = $wpdb->prefix . "contact_form";
        $wpdb->delete($contact_form_table, ['id' => $id], ['%d']);
    }
    if(isset($_GET['action']) && $_GET['action'] === 'delete' && isset($_GET['id'])){
        $id = intval($_GET['id']);
        contact_form_data_delete($id);
        echo "Data successfully deleted";
    }
?>
<!--Delete Programs from the table Ends -->